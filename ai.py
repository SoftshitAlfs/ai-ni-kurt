...
bot_whitelist = [123456789012345678, 987654321098765432]  # Users allowed to add bot
suspicious_patterns = [
...
async def on_guild_join(guild):
    ...
    if owner_id not in bot_whitelist:
        await guild.leave()
...
whitelist_users = [123456789012345678, 987654321098765432]
...
async def scan(ctx, url: str):
    ...
    if not stats:
        ...
        return
    embed = discord.Embed(title="VirusTotal Scan Result", color=discord.Color.blue())
    embed.add_field(name="Malicious", value=str(stats.get("malicious", 0)))
    embed.add_field(name="Suspicious", value=str(stats.get("suspicious", 0)))
    embed.add_field(name="Undetected", value=str(stats.get("undetected", 0)))
    embed.add_field(name="Timeout", value=str(stats.get("timeout", 0)))
    for engine, data in results.items():
        value = f"Category: {data.get('category')}\nResult: {data.get('result')}"
        embed.add_field(name=engine, value=value, inline=False)
    await msg.edit(content=None, embed=embed)
