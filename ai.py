...
async def send_log_message(bot, guild_id, member, action):
    ...
    try:
        ...
        embed.set_footer(text="Made by Orca")
        embed.timestamp = discord.utils.utcnow()
        embed.set_thumbnail(url=member.avatar.url if member.avatar else member.default_avatar.url)
        embed.set_image(url="https://cdn.discordapp.com/attachments/1379846639069691965/1381888389850333296/orca2.jpg?ex=6864d6da&is=6863855a&hm=f7ab771cea2f0317ec01a4d3f18e5601e39c23b5079c6382e16957a8a3b62191&")
...
async def on_ready():
    logger.info(f'Logged in as {bot.user} (ID: {bot.user.id})')
    logger.info('------')
    activity = discord.Game(name="tite")
    await bot.change_presence(status=discord.Status.online, activity=activity)
...
async def scan(ctx, url: str):
    ...
    if result:
        ...
        embed.add_field(name="Suspicious detections", value=f" {suspicious}", inline=True)
